...
export function Head({columns}){
    ...
    ? <div key={i}  className="flex items-center justify-center">
        ...
        <h2 className="py-3 ml-2 font-semibold text-xs md:text-sm lg:text-base"> {column} </h2>
    </div>
...
export function ModificationButtons({myId, title, triggerRefresh, editableRowId, onClickModificationBtn, onClickSaveBtn, }){
    ...
    return (
        <div key={7} className="flex items-center justify-evenly text-base text-center">
            {toggle === false && myId === editableRowId
            ? (
                <button key={0} onClick={() => {setToggle(true); onClickSaveBtn(myId)}} className='no-underline text-green-500'><img src='../icons/check.svg' alt='check' /></button>
            )
            : (
                <button key={0} onClick={(e) => {setToggle(false); ;e.preventDefault(); onClickModificationBtn(myId)}} className='no-underline text-amber-500'><img src='../icons/edit.svg' alt='edit' /></button>
            )}
            
            <button key={1} onClick={ async (e) =>{
                e.preventDefault();
